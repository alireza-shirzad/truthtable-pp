...
We start with the approach of $\truthtable$~\cite{cryptoeprint:2026/1260} which arithmetizes a string-typed column as a single hash column $(\act,\mathsf{h})$, which we adopt as our main column. Hashing, however, is opaque: beyond equality testing---which it preserves---it supports no operation on the underlying strings. To enable white-box operations on the string contents, we augment the hash column with a group of auxiliary columns. These auxiliary columns come in two lengths:
...
\begin{itemize}[nosep]
    \item \emph{Length column $\mathsf{l}$}, with $\mathsf{l}[i]$ being the length $|\col[i]|$ of the $i$-th string in $\col$.
    \end{itemize}
   ...
   \begin{itemize}[nosep]
    \item \emph{Character column $\mathsf{char}$} contains the concatenation of all strings in $\col$.
    ...
    \item \emph{Boundary marker column $\mathsf{bnd}$}, with $\mathsf{bnd}[j]$ being $1$ if the $j$-th character of $\mathsf{char}$ is the first character of a string (a string boundary), and $0$ otherwise.
    \end{itemize}
